...
function exportCSRDMatrix() {
    ...
    rows.push([c('faostat_crop_climate_adjustment: when non-empty, cc_total_kg_co2e/cc_per_kg_kg_co2e above already include a country-specific ratio (origin_factor/FR_reference_factor) from FAOSTAT GCE (Crop Residues, Burning, Rice Cultivation, Synthetic Fertilizers elements) + QCL (Production, Area harvested), FAO TIER 1, 2023. Column value discloses per-row whether fertilizer is included for that specific ingredient (\'...full factor)\' vs \'...partial factor)\') since FAOSTAT fertilizer/area-harvested data availability varies by country -- see PDF Layer B8b for which applies to this ingredient. Excludes on-farm fuel/machinery in all cases, which FAOSTAT does not publish at crop-specific resolution. Empty = not applied, either because this ingredient is not one of the mapped crops, origin is FR, primary data was supplied, or the computed ratio/inputs failed a plausibility check (see full derivation in PDF report, Layer B8b/B8c) -- in all not-applied cases, cc_total/cc_per_kg reflect the unmodified AGRIBALYSE FR-reference value.')]);
    ...
    ingComps.forEach(ing => {
        ...
        const faostatCropNote = (fcAdj && fcAdj.applied)
            ? 'YES (crop=' + fcAdj.crop_key + ', ratio=' + fcAdj.ratio_applied.toFixed(4) +
              'x, FAOSTAT GCE+QCL 2023, ' + (fcAdj.fertilizer_included ? 'full factor incl. fertilizer' : 'partial factor') + ')'
            : '';
