    ...
    return {
        ...
        ingredients: ingComps.map(ing => {
            ...
            return {
                ...
                // FIX EUDR-1 (continued): kept as a short, exact-matchable enum -- every
                // retailer formatter below does `i.eudrRisk === 'HIGH'` string equality
                // checks. The real EC system has 3 tiers (low/standard/high), but AIOXY
                // does not have a complete verified list of the ~50 "standard" countries,
                // so the non-high case is honestly labeled 'NOT_HIGH' rather than falsely
                // asserting 'LOW' precision it doesn't have. Full citation and dispute
...
function generateSainsburysCSV(d) {
    ...
    rows.push([c("NOTICE: This template is built from AIOXY's best available reference to the SSQ structure, not a")]);
    rows.push([c("published public schema — Sainsbury's SSQ is delivered via an authenticated supplier portal with no")]);
    rows.push([c("public machine-readable format. Confirm exact field names/order against your live portal before submission.")]);
    rows.push(['']);
...
function generateLidlCSV(d) {
    ...
    rows.push([c('NOTICE: Built from AIOXY\'s best available reference to Lidl\'s questionnaire structure, not a published')]);
    rows.push([c('public schema — Lidl\'s CSR questionnaire is delivered via supplier portal with no public machine-readable')]);
    rows.push([c('format. Confirm exact field names/order against your live portal before submission.')]);
    rows.push(['']);
...
function generateAldiCSV(d) {
    ...
    rows.push([c('NOTICE: Built from AIOXY\'s best available reference to Aldi\'s questionnaire structure, not a published')]);
    rows.push([c('public schema — Aldi\'s supplier questionnaire has no public machine-readable format. Confirm exact')]);
    rows.push([c('field names/order against your live buyer portal before submission.')]);
    rows.push(['']);
...
function generateReweCSV(d) {
    ...
    rows.push([c('NOTICE: Built from AIOXY\'s best available reference to Rewe\'s form structure, not a published public')]);
    rows.push([c('schema — Rewe\'s supplier form has no public machine-readable format. Confirm exact field names/order')]);
    rows.push([c('against your live supplier portal before submission.')]);
    rows.push(['']);
...
function generateAlbertHeijnCSV(d) {
    ...
    rows.push([c('NOTICE: Built from AIOXY\'s best available reference to Ahold Delhaize\'s data request structure, not a')]);
    rows.push([c('published public schema — this has no public machine-readable format. Confirm exact field names/order')]);
    rows.push([c('against your live supplier portal before submission.')]);
    rows.push(['']);
...
function generateCarrefourCSV(d) {
    ...
    rows.push([c("NOTICE: Built from AIOXY's best available reference to Carrefour's form structure, not a published")]);
    rows.push([c('public schema — this has no public machine-readable format. Confirm exact field names/order against')]);
    rows.push([c('your live supplier portal before submission.')]);
    rows.push(['']);
...
function generateLeclercCSV(d) {
    ...
    rows.push([c("NOTICE: Built from AIOXY's best available reference to E.Leclerc's questionnaire structure, not a")]);
    rows.push([c('published public schema. Confirm exact field names/order against your live supplier portal before')]);
    rows.push([c('submission.')]);
    rows.push(['']);
...
function generateIntermarcheCSV(d) {
    ...
    rows.push([c("NOTICE : construit a partir de la meilleure reference disponible chez AIOXY pour la structure du")]);
    rows.push([c('questionnaire Intermarche, pas un schema public officiel. Verifiez les noms de champs/ordre exacts')]);
    rows.push([c('aupres de votre portail fournisseur avant soumission.')]);
    rows.push(['']);
...
function generateCoopCHCSV(d) {
    ...
    rows.push([c("NOTICE: Built from AIOXY's best available reference to Coop's data form structure, not a published")]);
    rows.push([c('public schema. Confirm exact field names/order against your live supplier portal before submission.')]);
    rows.push(['']);
...
function generateCSRD_ESRS_CSV(d) {
    ...
    rows.push([c('Standard: European Sustainability Reporting Standards (EFRAG) ESRS E1-E5')]);
    ...
    rows.push([c('VERSION NOTICE: A revised ESRS delegated act was adopted by the European Commission on 3 July 2026')]);
    rows.push([c('(Omnibus simplification). The revised standard applies from FY2027, with optional early use for FY2026.')]);
    rows.push([c('EFRAG\'s companion 2026 datapoint list is still in draft/comment period (comments close 23 Oct 2026)')]);
    rows.push([c('and is not yet final. Field mapping below follows the 2023 standard. CONFIRM with your ESG/reporting')]);
    rows.push([c('team which ESRS version applies to your filing period before submission — do not assume this file')]);
    rows.push([c('reflects the revised 2026/2027 standard.')]);
...
function generateCDP_SC_CSV(d) {
    ...
    rows.push([c('VERSION NOTICE: Field mapping is based on the 2024 module structure. CDP revises module numbering')]);
    rows.push([c('each disclosure cycle — confirm the current module reference against your live CDP account before')]);
    rows.push([c('submission.')]);
    rows.push(['']);
