...
function generateQRCode(total, materials, energy, transport, country, details) {
    ...
    // Create simple text version for QR code
    const qrText = `AIOXY|${productName}|CO2:${total.toFixed(2)}kg|Saved:${savedCO2.toFixed(2)}kg|Mats:${
        details.materials.map(m => `${m.name.substring(0,3)}:${m.weight}kg`).join(',')
    }|Energy:${details.energy.amount}kWh|${country.substring(0,3)}`;
    ...
    try {
        ...
        qrError.textContent = "QR generation failed - please try again";
