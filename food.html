...
<head>
    ...
    <style>
        * {
        ...
        .ingredient-list {
            /* FIXED: Remove fixed max-height and overflow constraints */
            border: 1px solid var(--border);
            border-radius: 12px;
            padding: 1rem;
            margin-top: 1.5rem;
        ...
        .metric-card {
            ...
            position: relative;
            /* FIXED: Remove overflow hidden that was clipping content */
        }
        ...
        .environmental-story {
            ...
            box-shadow: var(--shadow-lg);
            position: relative;
            /* FIXED: Remove overflow hidden that was clipping text */
        }
        ...
        .story-content {
            ...
            position: relative;
            z-index: 2;
            /* FIXED: Ensure text can flow naturally */
        }
        ...
        .comparison-card {
            ...
            box-shadow: var(--shadow-md);
            border-left: 4px solid var(--secondary);
            /* FIXED: Remove overflow constraints */
        }
        ...
        .audit-trail-section {
            ...
            padding: 1.5rem;
            margin: 1.5rem 0;
            /* FIXED: Remove max-height and overflow constraints */
        }
            ...
            <div id="results-tab" class="tab-content hidden">
                <div class="card">
                    ...
                    <div id="resultsContent">
                        ...
                        <!-- NEW: Enhanced Storytelling Section -->
            ...
            <div id="business-case-tab" class="tab-content hidden">
                <div class="card">
                    <div class="card-header">
                        ...
                        <div class="badge">
                            <i class="fas fa-chart-line"></i>
                            Financial Impact • ROI • Market Advantage
                        </div>
                    </div>

                    <div class="business-case">
                        <h3 style="margin-bottom: 1.5rem; color: var(--primary); text-align: center;">
                            Sustainability Business Case Analysis
                        </h3>
                        
                        <div class="business-metrics">
                            <div class="business-card">
                                <i class="fas fa-euro-sign" style="font-size: 2rem; color: var(--secondary); margin-bottom: 1rem;"></i>
    ...
    <script>
        ...
        function detectProductCategory(productName, ingredients, selectedCategory) {
            ...
            if (name.includes('burger') || name.includes('patty') || name.includes('sausage')) {
                // FIX: Plant-based burgers should be compared to beef, not cheese
                return 'beef-product';
            }
        ...
        function detectFoodType(ingredients) {
            ...
            // Plant-based default - FIX: Compare to beef, not bread
            return 'beef-product';
        }
        ...
        const foodCalculationEngine = {
            ...
            calculateFoodImpact() {
                ...
                // SMART CATEGORY DETECTION - FIXED
        ...
        function calculateBusinessCase(results) {
            ...
            // Business metrics
            const annualProduction = 10000; // kg per year (example)
            const carbonPrice = 50; // € per ton CO2
            const premiumPotential = reductionPercentage * 0.5; // 0.5% price premium per 1% reduction
            const marketGrowth = Math.min(reductionPercentage * 2, 25); // Market share growth
            
            // ROI calculation
            const implementationCost = 5000; // € one-time cost
            const annualSavings = (co2Reduction * annualProduction * carbonPrice) / 1000;
            const annualRevenueIncrease = (premiumPotential / 100) * (annualProduction * 10); // Assuming €10/kg price
            const totalAnnualBenefit = annualSavings + annualRevenueIncrease;
            const roi = ((totalAnnualBenefit * 3) - implementationCost) / implementationCost * 100; // 3-year ROI
            ...
            return {
                ...
                carbonReduction: co2Reduction * annualProduction,
        ...
        function getProductTypeDescription() {
            ...
            if (productName.includes('tshirt') || name.includes('shirt')) return 'clothing item';
        ...
        function updateBusinessCase() {
            if (!finalPefResults || Object.keys(finalPefResults).length === 0) {
                document.getElementById('businessStory').innerHTML = `
                    <div class="story-highlight">
                        <i class="fas fa-calculator"></i>
                        <strong>Calculate environmental impacts first</strong> to see the business case analysis
                    </div>
                `;
                return;
            }
            ...
            const businessCase = calculateBusinessCase(results);

            // Update business metrics
            document.getElementById('roiValue').textContent = businessCase.roi.toFixed(0) + '%';
            document.getElementById('savingsValue').textContent = '€' + businessCase.annualSavings.toFixed(0);
            ...
            document.getElementById('businessStory').innerHTML = `
                <div class="story-highlight">
                    <i class="fas fa-euro-sign"></i>
                    <strong>€${businessCase.totalAnnualBenefit.toFixed(0)} annual benefit</strong> - combining carbon savings and premium pricing
                </div>
                <div class="story-highlight">
                    <i class="fas fa-chart-line"></i>
                    <strong>${businessCase.marketGrowth.toFixed(1)}% market share growth</strong> - capturing sustainability-conscious consumers
                </div>
                <p>
                    This sustainability investment delivers a <strong>${businessCase.roi.toFixed(0)}% 3-year ROI</strong> with 
                    <strong>€${businessCase.annualSavings.toFixed(0)} annual cost savings</strong> from carbon reduction and 
                    <strong>${businessCase.premiumPotential.toFixed(1)}% price premium potential</strong>.
                </p>
            `;
        }
